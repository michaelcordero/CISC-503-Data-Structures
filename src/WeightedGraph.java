...
public class WeightedGraph<K extends Comparable<K>,V> implements Graph<K,V> {
    ...
    public void shortestPath(K fromVertexKey, K toVertexKey) {
        ...
        PriorityQueue<DijkstraTable<K,V>> pq = new PriorityQueue<>(this.size(), Comparator.comparing(DijkstraTable::getDistance));
        ...
        while(!pq.isEmpty()) {
            ...
            current.getVertex().adjacencyList().forEach(v -> {
                ...
                // when any positive int is added to Integer.MAX_VALUE, it ends up being negative.
                // to fix this bug, we don't change the value and leave as is.
                int computed_distance = current.getDistance() + to_weight < 0 ? current.getDistance() : current.getDistance() + to_weight ;
